...
async function acoustidHandler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!key) {
    res.status(500).json({ error: { code: 'INTERNAL_ERROR', message: 'Server missing ACOUSTID_API_KEY' } });
    return;
  }

  const { fingerprint, duration } = req.query;
  if (!fingerprint || !duration) {
    res.status(400).json({ error: { code: 'BAD_REQUEST', message: 'Missing fingerprint or duration' } });
    return;
  }
  ...
  try {
    ...
    const response = await fetchWithTimeout(`${ACOUSTID_URL}?${params.toString()}`, {
    ...
    const normalized = normalizeError(e, { code: 'INTERNAL_ERROR', message: 'AcoustID proxy failed' });
