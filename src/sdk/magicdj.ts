...
async function fetchJSON<T>(
  ...
  if (!res.ok) {
    ...
    throw new AppError('UPSTREAM_ERROR', `Request failed: ${res.status}`, {
...
export const magicdj = {
  async generateMagicSet(params: {
    ...
    const body = JSON.stringify({
      ...params,
      trackCount: Math.min(params.trackCount ?? 10, 20),
    });
